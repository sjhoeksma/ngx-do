...
import { BaseAuth } from './auth-services/base.auth';
import { DoProxyAuth } from './auth-services/do-proxy.auth';
import { MsalAuth } from './auth-services/msal.auth';
  ...
  public get authService(): AuthInterface { // Create authentication service on the fly
    if (!this._authService || this._authBackend !== this.coreConfig.backend) {
      ...
      switch (this.coreConfig.backendValue('type')) {
        ...
        case 'msal':
         this._authService = new MsalAuth(this.coreConfig, this.rest); break;
        default:
